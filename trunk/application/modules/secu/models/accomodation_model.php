...
class Accomodation_model extends  Model {
    ...
    public function get_accomodation_list($limit,$start) {
        ...
        $accomodation = $this->db->query($accomodation_sql);
         try{
            $accomodation_sql = "SELECT *
                                    FROM tbl_accomodations";
            $accomodation_sql.= " limit ".$start.",".$limit;
        $accomodation = $this->db->query($accomodation_sql);
...
public function get_accomodation_list_forhotelform() {
        $accomodation_sql = "SELECT *
                                    FROM tbl_accomodations";
        $accomodation = $this->db->query($accomodation_sql);
         try{
    ...
    public function get_accomodation_list_count() {
         try{
            $accomodation_sql = "SELECT count(*) as count
                                    FROM tbl_accomodations";
        $accomodation = $this->db->query($accomodation_sql);
        if($accomodation->num_rows() > 0) {
                  $accomodation_result = $accomodation->row_array();
                  return $accomodation_result["count"];
            } else {
                return FALSE;
            }
        } catch(Exception $e){
